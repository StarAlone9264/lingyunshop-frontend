          ...
          <div :style="{ height: tableScreenHeight + 'px' }">
            <table id="dataList" class="table table-bordered table-striped table-hover dataTable">
              ...
              <tr v-for="(innerItem,index) of list">
                ...
                <td>
                  <span v-if="innerItem.status === '1'">审核通过</span>
                  <span v-if="innerItem.status === '0'">未审核</span>
                  <span v-if="innerItem.status === '2'">审核未通过</span>
                  <span v-if="innerItem.status === '3'">关闭</span>
                </td>
      ...
      postRequest("/tbSeller/update",params)
          .then(res => {
            if (res.data.message === 'success'){
              this.setNotify('tip', '操作成功', 'success');
              this.handleList();
            }else {
